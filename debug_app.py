from flask import Flask, render_template, request, redirect, url_for, flash
import sqlite3
import os
import traceback

app = Flask(__name__)
app.secret_key = 'your_secret_key_here'

# Database setup
DATABASE = 'school_reviews.db'

def init_db():
    """Initialize the database"""
    try:
        db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), DATABASE)
        print(f"Initializing database at: {db_path}")
        
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # Create table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS reviews (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                school_name TEXT NOT NULL,
                reviewer_name TEXT NOT NULL,
                rating INTEGER NOT NULL,
                comment TEXT NOT NULL
            )
        ''')
        conn.commit()
        conn.close()
        print("✓ Database initialized successfully!")
        return True
    except Exception as e:
        print(f"✗ Database initialization failed: {e}")
        traceback.print_exc()
        return False

def get_db():
    """Get database connection"""
    try:
        db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), DATABASE)
        print(f"Connecting to database at: {db_path}")
        
        conn = sqlite3.connect(db_path)
        conn.row_factory = sqlite3.Row
        print("✓ Database connection successful!")
        return conn
    except Exception as e:
        print(f"✗ Database connection failed: {e}")
        traceback.print_exc()
        return None

@app.route('/')
def index():
    print("Accessing index route")
    return redirect(url_for('reviews'))

@app.route('/addreview', methods=['GET', 'POST'])
def add_review():
    print(f"Accessing add_review route with method: {request.method}")
    
    if request.method == 'POST':
        print("Processing POST request")
        school_name = request.form['school_name']
        reviewer_name = request.form['reviewer_name']
        rating = request.form['rating']
        comment = request.form['comment']
        
        print(f"Form data: {school_name}, {reviewer_name}, {rating}")
        
        # Validation
        if not all([school_name, reviewer_name, rating, comment]):
            print("Validation failed: missing fields")
            flash('All fields are required!', 'error')
            return render_template('add_review.html')
        
        try:
            rating = int(rating)
            if not 1 <= rating <= 5:
                print("Validation failed: invalid rating")
                flash('Rating must be between 1 and 5!', 'error')
                return render_template('add_review.html')
        except ValueError:
            print("Validation failed: rating not a number")
            flash('Rating must be a number!', 'error')
            return render_template('add_review.html')
        
        # Save to database
        print("Attempting to save to database...")
        conn = get_db()
        if conn is None:
            print("✗ Database connection failed in add_review")
            flash('Database connection error!', 'error')
            return render_template('add_review.html')
        
        try:
            cursor = conn.cursor()
            cursor.execute(
                'INSERT INTO reviews (school_name, reviewer_name, rating, comment) VALUES (?, ?, ?, ?)',
                (school_name, reviewer_name, rating, comment)
            )
            conn.commit()
            conn.close()
            print("✓ Review saved successfully!")
            flash('Review submitted successfully!', 'success')
            return redirect(url_for('reviews'))
        except Exception as e:
            print(f"✗ Error saving review: {e}")
            traceback.print_exc()
            flash('Error saving review. Please try again.', 'error')
    
    print("Rendering add_review template")
    return render_template('add_review.html')

@app.route('/reviews')
def reviews():
    print("Accessing reviews route")
    
    conn = get_db()
    if conn is None:
        print("✗ Database connection failed in reviews")
        flash('Database connection error!', 'error')
        return render_template('reviews.html', reviews=[])
    
    try:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM reviews ORDER BY id DESC')
        reviews_list = cursor.fetchall()
        conn.close()
        print(f"✓ Successfully fetched {len(reviews_list)} reviews")
        return render_template('reviews.html', reviews=reviews_list)
    except Exception as e:
        print(f"✗ Error fetching reviews: {e}")
        traceback.print_exc()
        flash('Error loading reviews.', 'error')
        return render_template('reviews.html', reviews=[])

if __name__ == '__main__':
    print("=== Starting Debug Flask App ===")
    if init_db():
        print("✓ Database ready, starting Flask app...")
        app.run(debug=True, host='127.0.0.1', port=5000)
    else:
        print("✗ Failed to initialize database, cannot start app") 